...
/************************************************
*函数名:	convert_from_bytes_to_power_of_two
...
*输入：clkdiv:时钟分频系数
*输出：无
*备注：CK时钟 = SDIOCLK/[clkdiv+2];(SDIOCLK时钟固定为48Mhz)
************************************************/
...
*输入：无
*输出：错误代码;(0,无错误)
*备注：查询所有SDIO接口上的卡设备,并查询其电压和配置时钟
************************************************/
...
*功能：	设置SDIO总线宽度
*输入：	wmode:位宽模式	SDIO_BusWide_1b：1位数据宽度; (默认)
*						SDIO_BusWide_4b：4位数据宽度; 
*						SDIO_BusWide_8b：8位数据宽度(Only for MMC)
*输出：	sd卡错误码
*备注：	MMC卡不支持4bit模式
************************************************/
...
*输入：	addr:卡的RCA地址
*输出：	sd卡错误码
*备注：	选择相对地址(rca)为addr的卡,取消其他卡.如果为0,则都不选择.
************************************************/
...
*		bufsize:传输数据量
*		dir:方向;
			DMA_DIR_MemoryToPeripheral 存储器-->SDIO(写数据);
			DMA_DIR_PeripheralToMemory SDIO-->存储器(读数据);
*输出：	NONE
*备注：	NONE
************************************************/
